  ...
  return <div className="hz-settings-page hz-platform-settings d-flex flex-column gap-4">
    ...
    <Card title={editingCompanyId ? 'Edit company' : 'Add company'} subtitle="Create a tenant before assigning its plan.">
      <form className="row g-3 align-items-end" onSubmit={(event) => { event.preventDefault(); saveCompany.mutate({ ...company, productiveThresholdPercent: Number(company.productiveThresholdPercent), neutralThresholdPercent: Number(company.neutralThresholdPercent) }); }}>
        <div className="col-md-5"><label className="form-label">Company name<input required className="form-control" value={company.name} onChange={(event) => setCompany({ ...company, name: event.target.value })} /></label></div>
        <div className="col-md-5"><label className="form-label">Domain<input className="form-control" placeholder="example.com" value={company.domain} onChange={(event) => setCompany({ ...company, domain: event.target.value })} /></label></div>
        <div className="col-md-2"><label className="form-label">Productive %<input type="number" min="1" max="100" className="form-control" value={company.productiveThresholdPercent} onChange={(event) => setCompany({ ...company, productiveThresholdPercent: event.target.value })} /></label></div>
        <div className="col-md-2"><label className="form-label">Neutral %<input type="number" min="0" max="99" className="form-control" value={company.neutralThresholdPercent} onChange={(event) => setCompany({ ...company, neutralThresholdPercent: event.target.value })} /></label></div>
    ...
    <Card title={editingSubscriptionId ? 'Edit subscription' : 'Assign subscription'} subtitle="Subscription limits are enforced by the backend.">
      <form className="row g-3 align-items-end" onSubmit={(event) => { event.preventDefault(); saveSubscription.mutate({ id: editingSubscriptionId, companyId: Number(selectedCompany), payload: { ...subscription, employeeLimit: Number(subscription.employeeLimit), deviceLimit: Number(subscription.deviceLimit), amount: subscription.amount ? Number(subscription.amount) : null, startDate: subscription.startDate || null, renewalDate: subscription.renewalDate || null } }); }}>
        <div className="col-md-3"><label className="form-label">Company<select required className="form-select" value={selectedCompany} onChange={(event) => setSelectedCompany(event.target.value)}><option value="">Select company</option>{(companies.data || []).map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label></div>
        <div className="col-md-2"><label className="form-label">Plan<select className="form-select" value={subscription.plan} onChange={(event) => setSubscription({ ...subscription, plan: event.target.value })}>{plans.map((item) => <option key={item}>{item}</option>)}</select></label></div>
        <div className="col-md-2"><label className="form-label">Status<select className="form-select" value={subscription.status} onChange={(event) => setSubscription({ ...subscription, status: event.target.value })}>{statuses.map((item) => <option key={item}>{item}</option>)}</select></label></div>
        <div className="col-md-2"><label className="form-label">Employees<input type="number" min="1" className="form-control" value={subscription.employeeLimit} onChange={(event) => setSubscription({ ...subscription, employeeLimit: event.target.value })} /></label></div>
        <div className="col-md-2"><label className="form-label">Devices<input type="number" min="1" className="form-control" value={subscription.deviceLimit} onChange={(event) => setSubscription({ ...subscription, deviceLimit: event.target.value })} /></label></div>
